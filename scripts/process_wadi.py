...
def downsample(data, labels, down_len):
    ...
    d_labels = np_labels[:down_time_len*down_len].reshape(-1, down_len).astype(int)
    # if exist anomalies, then this sample is abnormal
    # d_labels = np.round(np.max(d_labels, axis=1))
    d_labels = np.round(np.median(d_labels, axis=1))
...
def main():
    ...
    train = pd.read_csv('./data/wadi/WADI_14days.csv', index_col=0)
    test = pd.read_csv('./data/wadi/WADI_attackdata_labelled.csv', index_col=0)
    ...
    train_df.to_csv('./data/wadi/train.csv')
    test_df.to_csv('./data/wadi/test.csv')
