...
## edge index calculation for all edge relation in a batch
...
## Prediction or reconstruction module for getting final output from node embedding
...
class GLM(nn.Module):
    def __init__(self, edge_index_sets, node_num, dim=64, out_layer_inter_dim=256, input_dim=10, out_layer_num=1, a_init=1):
        ...
        self.init_params()
    ## initialize adjancency matrix with a constant term
    ...
    def forward(self, data, org_edge_index):
        ...
        for i, edge_index in enumerate(edge_index_sets):
            ...
            ##get node embedding Z by learning intervariable dependencies along with edge weights
        ...
        ## finding reconstruction and prediction output from node embedding
        out_recons = self.out_layer_recons(x)                            
        out_recons = out_recons.view(-1, node_num, self.input_dim)
        out_pred = self.out_layer_pred(x) 
        out_pred = out_pred.view(-1, node_num)
